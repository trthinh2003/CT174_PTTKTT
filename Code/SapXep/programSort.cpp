...
void giaoDien() {
	printf("\n---------CHAO MUNG BAN DEN VOI CHUONG TRINH SAP XEP---------\n");
...
void runProgram() {
	...
	int choice;
	printf("\n->Nhap vao lua chon cua ban: "); scanf("%d%*c", &choice);
	switch (choice) {
...
//	ShellSort(a, n);
	...
	printf("Thoi gian chay cua chuong trinh la: %.5lfs", usedtime);
}
...
int main() {
	while (1) {
		...
		char continueChoice;
		printf("->Lua chon cua ban la: "); scanf("%c%*c", &continueChoice);
		if (continueChoice == 'Y') {
			system("cls");
			continue;
		}
