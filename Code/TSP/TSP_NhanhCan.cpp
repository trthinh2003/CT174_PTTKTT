...
void in_ma_tran(canh a[][size], int n) {
	...
	for (i = 0; i < n; i++) {
		for(j = 0; j < n; j++) {
			printf(" %c%c = %5.2f  ", a[i][j].dau + 65, a[i][j].cuoi + 65, a[i][j].do_dai);
		}
		printf("\n");
	}
}
	 	...
	 	// Tinh 2 dai luong cua mot nut trong
			...
			*CD = can_duoi(a, *TGT, n, i + 1);
			...
			if (i == n-2) { // Da xet het tat ca cac dinh nhung duong di moi co n-1 canh
		 	...
		 	else Nhanh_Can(a, n, i + 1, j, TGT, CD, GNNTT, x, PA); //Xet nut con cua nut i
	 	}
...
int main() {
	...
	while (1) {
		...
		printf("\nBan muon xuat phat tu thanh pho nao? ");
		printf(" (Nhap mot trong cac thanh pho tu A den %c): ", n - 1 + 65);
		scanf("%c", &tpxp);
